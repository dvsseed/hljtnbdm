...
class CreateCaseCaresTable extends Migration
{
    ...
    public function up()
    {
        Schema::create('casecare', function (Blueprint $table) {
            ...
            $table->integer('cc_usebsm')->unsigned()->default(0);
            ...
            $table->timestamps();
            $table->foreign('patientprofile1_id')->references('id')->on('patientprofile1');
            $table->foreign('cc_patientid')->references('pp_patientid')->on('patientprofile1');
            // $table->foreign('cc_usebsm')->references('id')->on('bsm');
        });
    }
